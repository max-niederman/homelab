...
if [[ "${APPLICATION}" == "rtorrent" ]]; then
	...
	incoming_ports_lan_array+=(8112 58846)
fi
...
# split comma separated string into array from DOCKER_NETWORK env variable
IFS=',' read -ra docker_network_array <<< "${DOCKER_NETWORK}"
...
for docker_network_item in "${docker_network_array[@]}"; do

  # accept input to/from docker containers on custom subnets
  iptables -A INPUT -s "${docker_network_item}" -d "${docker_network_item}" -j ACCEPT
...
for vpn_remote_ip_item in "${vpn_remote_ip_array[@]}"; do
	...
	if [[ -z "${rule_exists}" ]]; then
		...
		# return rule
		iptables -A INPUT ! -i "${VPN_DEVICE_TYPE}" -s "${vpn_remote_ip_item}" -j ACCEPT
...
for incoming_ports_ext_item in "${incoming_ports_ext_array[@]}"; do
	...
	for vpn_remote_protocol_item in "${multi_protocol_array[@]}"; do
		...
		iptables -A INPUT ! -i "${VPN_DEVICE_TYPE}" -p "${vpn_remote_protocol_item}" --dport "${incoming_ports_ext_item}" -j ACCEPT
...
for lan_network_item in "${lan_network_array[@]}"; do
	...
	for incoming_ports_lan_item in "${incoming_ports_lan_array[@]}"; do
		...
		iptables -A INPUT ! -i "${VPN_DEVICE_TYPE}" -s "${lan_network_item}" -p tcp --dport "${incoming_ports_lan_item}" -j ACCEPT
	...
	for outgoing_ports_lan_item in "${outgoing_ports_lan_array[@]}"; do
		...
		# return rule
		iptables -A INPUT ! -i "${VPN_DEVICE_TYPE}" -s "${lan_network_item}" -p tcp --sport "${outgoing_ports_lan_item}" -j ACCEPT
...
for docker_network_item in "${docker_network_array[@]}"; do
  ...
  # accept output to/from docker containers on custom subnets
  iptables -A OUTPUT -s "${docker_network_item}" -d "${docker_network_item}" -j ACCEPT
...
for vpn_remote_ip_item in "${vpn_remote_ip_array[@]}"; do
	...
	if [[ -z "${rule_exists}" ]]; then
		...
		iptables -A OUTPUT ! -o "${VPN_DEVICE_TYPE}" -d "${vpn_remote_ip_item}" -j ACCEPT
...
for incoming_ports_ext_item in "${incoming_ports_ext_array[@]}"; do
	...
	for vpn_remote_protocol_item in "${multi_protocol_array[@]}"; do
		...
		# return rule
		iptables -A OUTPUT ! -o "${VPN_DEVICE_TYPE}" -p "${vpn_remote_protocol_item}" --sport "${incoming_ports_ext_item}" -j ACCEPT
...
for lan_network_item in "${lan_network_array[@]}"; do
	...
	for incoming_ports_lan_item in "${incoming_ports_lan_array[@]}"; do
		...
		# return rule
		iptables -A OUTPUT ! -o "${VPN_DEVICE_TYPE}" -d "${lan_network_item}" -p tcp --sport "${incoming_ports_lan_item}" -j ACCEPT
	...
	for outgoing_ports_lan_item in "${outgoing_ports_lan_array[@]}"; do
		...
		iptables -A OUTPUT ! -o "${VPN_DEVICE_TYPE}" -d "${lan_network_item}" -p tcp --dport "${outgoing_ports_lan_item}" -j ACCEPT
